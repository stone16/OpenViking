...
"""Durable local-source snapshots for background resource ingestion."""
...
_COPY_CONCURRENCY = 8
_IDENTITY_META = ("extension", "resolved_extension", "resolved_name", "original_filename")
...
async def stage_resource(
    ...
    *,
    ...
    """Copy one prepared local source into task-owned VikingFS temp storage."""
    path = resource.path
    ...
    return StagedResource(
        ...
        meta={key: resource.meta[key] for key in _IDENTITY_META if key in resource.meta},
    )
...
async def materialize_resource(
    ...
    *,
    ...
    """Restore a staged source to worker-local storage."""
    local_root = Path(tempfile.mkdtemp(prefix="ov_staged_resource_"))
    ...
    try:
        ...
        if staged.is_directory != local_path.is_dir() or not local_path.exists():
            raise ValueError("Staged resource source is missing")
    except BaseException:
